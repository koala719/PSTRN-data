...
class TensorRingLinear(nn.Module):
    ...
    def get_recover(self):
        weight = getattr(self, "node0")
        for i in range(1, self.nodes_num-1):
            weight_tmp = getattr(self, "node%d" % i)
            weight = torch.tensordot(weight, weight_tmp, dims=([-1], [0]))

        weight_tmp = getattr(self, "node%d" % (self.nodes_num-1))
        weight = torch.tensordot(weight, weight_tmp, dims=([0, -1], [-1, 0]))
        weight = weight.reshape(self.input_size, self.output_size).detach().numpy()
        return weight
