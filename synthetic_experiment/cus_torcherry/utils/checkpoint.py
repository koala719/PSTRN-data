...
class CheckLowestLoss(_CheckBase):
    def __init__(self):
        self.lowest_loss = math.inf

    def check2save(self, cls:Runner, epoch):
        if self.lowest_loss > cls.lowest_loss:
            self.lowest_loss = cls.lowest_loss
            print("Saving Lowest Loss Model...")
            torch.save(cls.model.state_dict(),
                       os.path.join(cls.model_dir, 'model-nn-lowest-loss.pt'))
            torch.save(cls.optimizer.state_dict(),
                       os.path.join(cls.model_dir, 'opt-nn-lowest-loss.tar'))
            torch.save(cls.lr_schedule.state_dict(),
                       os.path.join(cls.model_dir, 'lr-sdl-nn-lowest-loss.tar'))
